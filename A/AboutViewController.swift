    ...
    func addViewController (atOffset offset:CGFloat, dataForVC data:AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            if let headerStr = data as? String {
                ...
                if headerStr == "Contact Us" {
                    stackElementVC.textview = aboutusArray[0]
                  //  stackElementVC.stackimg = images[0]
                    
                } else if headerStr == "Company Goals" {
                    stackElementVC.textview = aboutusArray[1]
                 //   stackElementVC.stackimg = images[1]
                    
                }else if headerStr == "Company Services"{
                    stackElementVC.textview = aboutusArray[2]
                 //   stackElementVC.stackimg = images[2]
                }
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(AboutViewController.handlePan(gestureRecognizer:)))
